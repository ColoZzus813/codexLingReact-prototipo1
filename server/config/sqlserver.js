...
const jsonDatabasePath = path.resolve(__dirname, "../data/database.json");
...
export async function getConnection() {
  if (!pool) {
    pool = new sql.ConnectionPool(env.sqlServer);
    pool.on("error", (err) => {
...
export async function createDatabase() {
  const config = { ...env.sqlServer };
  delete config.database;
  
  const tempPool = new sql.ConnectionPool(config);
  await tempPool.connect();
  
  try {
    const request = tempPool.request();
    await request.query(`
      IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'CodexLing')
      CREATE DATABASE CodexLing;
    `);
    console.log("✅ Base de datos CodexLing verificada/creada");
  } finally {
...
async function readJsonDatabase() {
  try {
    const jsonText = await readFile(jsonDatabasePath, "utf8");
...
export async function ensureDatabase() {
  ...
  await request.query(`
    IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'app_state')
  ...
  const result = await request.query("SELECT content FROM app_state WHERE id = 1");
  if (result.recordset.length === 0) {
    // Intentar cargar datos del JSON si existen
    const jsonDatabase = await readJsonDatabase();
    const dataToInsert = jsonDatabase || initialDatabase;
    
    const insertReq = pool.request();
    insertReq.input('content', sql.NVarChar(sql.MAX), JSON.stringify(dataToInsert));
    await insertReq.query(`INSERT INTO app_state (id, content) VALUES (1, @content)`);
    console.log("✅ Datos iniciales cargados en SQL Server");
  }
}
...
export async function readDatabase() {
  ...
  const database = JSON.parse(result.recordset[0].content || "{}");
  return {
    users: [],
    userLevels: [],
    pythonLessons: [],
...
export async function writeDatabase(database) {
  ...
  const result = await selectReq.query("SELECT id FROM app_state WHERE id = 1");
  
  if (result.recordset.length > 0) {
    const updateReq = pool.request();
    updateReq.input('content', sql.NVarChar(sql.MAX), content);
    await updateReq.query(`UPDATE app_state SET content = @content WHERE id = 1`);
  } else {
    const insertReq = pool.request();
